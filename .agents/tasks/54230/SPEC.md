...
сделки для клиента в этом случае — владелец отеля. Практически речь про заказы
`OperatorOrganization` 89 (ООО «ГорТревел»), которая с 14.09.2026 работает с отелями от
имени их владельцев. Формально это указывается через шаблон
...
Для отельных заказов (`order.hotel?`, то есть `search_type == 'hotel'`) по России, созданных
с 14.09.2026 и принадлежащих `OperatorOrganization` 89 (ООО «ГорТревел»), если у отеля есть
засинканный из ФСА снапшот `HotelOrganization`, действовавший на дату создания заказа,
договор вместо блока туроператора печатает короткий блок реквизитов владельца отеля:
наименование (в обе строки — «полное» и «сокращённое», так как ФСА не разделяет эти формы),
реестровый номер отеля в реестре ФСА, адрес, ИНН, ОГРН (если он есть — у ИП его в данных
ФСА не бывает). Блок финансового обеспечения не показывается — он относится только к
туроператору. Блок
...
добавляется). Разовая rake-таска дозаполняет накопленные снапшоты только по тем отелям, по
которым заказы ГорТревела уже оформлены, — остальным новые поля приезжают с ближайшим
ночным снапшотом. Штатный синк в дозаполнении не участвует и, как и раньше, просто создаёт
новый снапшот при любом расхождении данных.
...
- `fsa_registry_entries` (таблица из LT-53621, полное зеркало реестра ФСА, синк —
  ...
  поэтому источником для самого синка она быть не может. Но для разового дозаполнения
  `ogrn`/`address` подходит: оба поля хранятся ровно так же, как их отдаёт живой эндпоинт
  (`entry['addressList']`, `entry['ownerOgrn']` — `app/services/fsa/registry_sync.rb:199,206`).
  Сверено по Replica 22.09.2026 на отеле из VCR-кассеты
  (`their_id 4566fc65-bafb-11f0-9990-f9c71843e436`): адрес совпал точным сравнением строк,
  ОГРН пуст и там, и там. Элементы `addresses` в зеркале несут ещё и `id`, которого в ответе
  живого эндпоинта нет, а у 31% записей адресов несколько — порядок между эндпоинтами
  непроверяем, см. «Implementation decisions».
...
- `OperatorOrganization` 89 — ООО «ГорТревел», ИНН 9721095485, `operator_id: 78`
  (Level.Travel, динамический оператор), `is_split: true`. Колонки под реестровый номер у
  `OperatorOrganization` нет (`id, inn, name, phone, active, operator_id, main, priority,
  is_split, external_id, vat, manager_priority, departures_mode`).
- `Article` `agreement_operator_info_78_operator_org_89` — v3 (start 2024-03-04) и v4
  (start 2025-08-01, dev-дамп). Блок v4 идёт строками «Полное наименование», «Сокращенное
  наименование», «Реестровый номер: РТО 022770», «Адрес (место нахождения)», «Адрес сайта в
  сети Интернет», «ИНН», «ОГРН» и далее четыре строки финансового обеспечения — отсюда
  взят порядок строк нашего блока.
- `hotels.register_record` (json) — запись отеля в реестре ФСА, заполняется
  `Hotels::RosreestrFsaUpdater#build_register_record`: ключ `id` содержит
  `hotel[:registerRecord]` — реестровый номер самого объекта размещения (формат
  `С232025000761`), `their_id` — идентификатор для запросов. В dev-дампе реестровая запись
  есть ровно у одного отеля и без `id`, поэтому покрытие поля на проде отдельно не
  проверено.
...
1. Отельный заказ ГорТревела по России, созданный не раньше 14.09.2026, флаг включён, у
   отеля есть `HotelOrganization` с заполненным ОГРН, начавший действовать не позже даты
   заказа — блок реквизитов показывает наименование (дважды — как «полное» и
   «сокращённое»), реестровый номер отеля из `register_record['id']`, адрес, ИНН и ОГРН;
...
2a. У отеля нет реестрового номера в `register_record` — строка «Реестровый номер» не
    выводится, остальной блок формируется как обычно.
...
6d. Отельный заказ ГорТревела, созданный раньше 14.09.2026, — печатается прежний текст
    туроператора (реквизиты самого ГорТревела в версии Article, активной на дату заказа).
6e. Отельный заказ по России после 14.09.2026, но `operator_organization` не 89 —
    поведение не меняется.
...
8. Синк: по сравнению с последним снапшотом изменилось любое из пяти полей (inn, name,
   phone_number, ogrn, address) — создаётся новый снапшот.
9. Синк: все пять полей совпадают с последним снапшотом — новый снапшот не создаётся
   (поведение дедупликации не ломается).
...
11. Таска: у снапшота пустые `ogrn`/`address`, ИНН совпадает с `owner_inn` записи зеркала —
    пустые поля дозаполняются из зеркала, непустые не трогаются, `active_from` не меняется,
    новых строк не появляется.
12. Таска: значения из зеркала нормализуются как в фетчере (`to_s.strip.presence`), иначе
    ночной синк счёл бы снапшот изменившимся и создал дубль.
12a. Таска: адрес длиннее прежнего лимита в 512 символов сохраняется целиком — колонка
    переведена в `text`.
12b. Таска: `their_id` сопоставляется без учёта регистра.
13. Таска: у отеля несколько снапшотов одного владельца с пустыми полями — дозаполняются
    все, а не только последний.
14. Таска: у снапшота другой ИНН, чем `owner_inn` в зеркале, — строка не трогается: это
    предыдущий владелец, его ОГРН и адрес взять неоткуда.
15. Таска: записи отеля в зеркале нет или она помечена `missing_since` — снапшот не
    трогается.

## Implementation decisions

**Миграция.** В `hotel_organizations` добавляются nullable `ogrn:string` и
`address:text`. Тип `text`, а не `string(512)`: адреса в реестре свободного формата и уже
сегодня доходят до 584 символов, а лимит 512 был нашей выдумкой, не требованием данных (по
зеркалу на 22.09.2026: максимум 584, восемь значений длиннее 400). Колонки на проде ещё нет,
поэтому тип поменян правкой той же миграции — без ALTER живой таблицы. `name` остаётся
`string(512)`: его пишет синк из export-эндпоинта, а там максимум 269 символов. Валидация
presence на новые поля не заводится — как и в данных ФСА, значение может отсутствовать;
`length` для `address` тоже не нужна, у `text` лимита нет. Переводы атрибутов добавляются в `config/locales/ru.yml`
...
сравнивает все пять полей (inn, name, phone_number, ogrn, address); любое расхождение даёт
новый снапшот, как и для трёх полей до задачи. Никакой логики дозаполнения в синке нет —
редактирование существующих строк живёт только в разовой таске.

**Разовое дозаполнение.** `lib/tasks/one_time/backfill_hotel_organization_details.rake`
(`rake one_time:backfill_hotel_organization_details`) берёт все снапшоты с пустым `ogrn` или
`address` и дозаполняет их из `fsa_registry_entries` — локального зеркала того же реестра.
Запросов к ФСА таска не делает вовсе и отрабатывает за секунды, поэтому обходит весь объём, а
не выборку по заказам.

Значения дописываются во **все** снапшоты отеля, у которых совпадает `inn`: ОГРН принадлежит
юрлицу, которое этот ИНН и определяет, и не меняется, датировать его нечем, а адрес мы и так
читаем текущий. Снапшоты с другим ИНН — предыдущие владельцы, их данных у нас нет и не
будет, они остаются пустыми. Пишутся только пустые поля, непустые не перезаписываются,
`active_from` не меняется, так что таска идемпотентна.

Детали, каждая из которых иначе тихо ломает результат:

- значения нормализуются ровно как в `Hotels::FsaHotelOrganizationFetcher`
  (`to_s.strip.presence`, адрес — `addresses[0]['name']`), иначе ночной синк счёл бы снапшот
  изменившимся и создал дубль;
- берутся только записи со `scope :present`, то есть подтверждённые реестром на последнем
  проходе синка (`missing_since` пуст); на 22.09.2026 помеченных пропавшими всего 2 из
  50 334, так что это страховка, а не фильтр;
- ключ сопоставления `their_id` приводится к нижнему регистру с обеих сторон:
  `Fsa::RegistrySync` его нормализует, `Hotels::RosreestrFsaUpdater` — нет, MySQL по своей
  коллации разницы не заметит, а поиск по ruby-хэшу молча промахнулся бы (сегодня все 17 909
  значений в нижнем регистре, но полагаться на это не на чем);
- запись идёт через `update!`, а не `update_columns`: колбэк `normalize_phone_number`
  идемпотентен (`PhoneFormatter.parse_phone` возвращает уже нормализованный `7XXXXXXXXXX` как
  есть), все валидируемые колонки в БД `NOT NULL` с теми же лимитами, так что существующие
  строки валидацию не завалят, а `updated_at` остаётся следом дозаполнения.

Остаточный риск, измеренный: два эндпоинта ФСА совпадают не всегда. Прямая сверка того, что
синк уже записал, с зеркалом (последний снапшот каждого отеля, ИНН совпадает, 17 170 пар):
`name` побайтово совпадает у 16 571 и различается у 599 — согласие 96,5%. Крайний пример: у
одного отеля в зеркале `owner_name` на 755 символов, а снапшот из export-эндпоинта хранит
«Индивидуальный предприниматель Орлова Ирина Алексеевна» (54 символа). Вдобавок у 31% записей
зеркала адресов несколько, а и фетчер, и таска берут первый, — совпадение порядка проверить
нечем, живой запрос требует ключа ФСА, которого нет в dev-окружении.

Значит по нескольким процентам отелей ночной синк всё-таки создаст новый снапшот — то есть
ровно то поведение, которое было бы без таски, — а в договоре до этого момента будет другой,
но настоящий адрес того же объекта размещения. Ожидание: вместо ~17 тысяч лишних строк
порядка нескольких сотен. Точная доля видна по числу новых снапшотов после первого ночного
прогона.

**Вывод в договор.** В `OrderDecorator#operator_info` добавляется ранняя ветка: если заказ
подходит (флаг `Settings` включён, `hotel?`, `object.created_at.to_date >=
HOTEL_OWNER_DETAILS_SINCE`, `package.operator_organization_id ==
HOTEL_OWNER_DETAILS_ORGANIZATION_ID`, `country&.id == Place::RUSSIA_ID`)
...
`hotel_organization.name`; реестровый номер — `package.hotel.register_record['id']`, то есть
номер самого объекта размещения в реестре ФСА (строка опускается, если его нет);
адрес (место нахождения) — `hotel_organization.address`; ИНН — `hotel_organization.inn`; ОГРН —
`hotel_organization.ogrn`, строка полностью опускается, если пусто; адрес — по тому же
правилу. Собственного заголовка блок не
...
2. **Обновление снапшота.** `spec/workers/hotel_organizations/sync_worker_spec.rb` —
   ...
   отличается именно `ogrn` или `address` при равенстве остальных полей.
2a. **Разовая таска.** `spec/lib/tasks/backfill_hotel_organization_details_spec.rb`, по
   образцу существующего `backfill_payment_card_numbers_spec.rb`: дозапись из записи зеркала
   без смены `active_from` и без новых строк, нормализация значений (пробелы по краям
   срезаются), заполнение только пустого поля при непустом соседнем, заполнение всех
   снапшотов одного владельца, пропуск снапшота предыдущего владельца и пропуск записи,
   помеченной `missing_since`.
3. **Рендер в договоре.** `spec/decorators/order_decorator_spec.rb` — новый
   ...
   `receive_messages` на `hotel?`, `package`, `country`, флаг), покрывает сценарии 1-6e:
...
- Общий распределённый rate limiter/`ExternalRequest` для
  ...
  явно не эта задача.
- Использование `fsa_registry_entries` в самом синке `HotelOrganizations::SyncWorker` —
  телефона в зеркале нет, снапшот из него не собрать; синк по-прежнему ходит живым запросом.
  В разовой таске дозаполнения зеркало используется, обоснование в «Current system state».
...
### Ответы постановщика (22.09.2026)

Вопрос задавался про пустые `ogrn`/`address`, ответ пришёл шире и сузил объём задачи.

- **Подменяем только заказы ГорТревела** (`package.operator_organization_id == 89`), а не
  все российские отельные заказы, как было в спеке до этого.
- **Только заказы, созданные с 14.09.2026** (именно дата создания заказа). Заказам раньше
  этой даты печатаются прежние реквизиты самого ГорТревела; новую версию `Article` под это
  заводить не надо — старый путь и так выбирает версию по дате заказа.
- **Пустые `ogrn` и `address` — идём старым путём.** То есть `hotel_owner_details_known?`
  остаётся как есть, вариант «печатать наименование и ИНН без адреса» отклонён.
- **Добавить строку «Реестровый номер»** после «Сокращенное наименование», «как сейчас по
  старой схеме пишется».

Отсюда в `OrderDecorator` появились три константы: `HOTEL_OWNER_DETAILS_ORGANIZATION_ID`
(89), `HOTEL_OWNER_DETAILS_SINCE` (`Date.new(2026, 9, 14)`) и
`HOTEL_OWNER_DETAILS_REGISTRY_NUMBER` (`'РТО 022770'`). Реестровый номер именно константой,
потому что поля под него нет нигде в данных — в блоке ГорТревела он лежит текстом внутри
`Article`, — а организация в схеме одна.

**Требует подтверждения постановщика:** под «реестровым номером» взят собственный номер РТО
ГорТревела, строкой из его же `Article` v4. Формулировка «как сейчас по старой схеме
пишется» допускала и вторую трактовку — реестровый номер самого отеля в реестре ФСА
(`hotel.register_record['id']`, формат `С232025000761`), он у нас тоже есть. Выбрана первая,
потому что «как сейчас пишется» — это буквально строка `Реестровый номер: РТО 022770` из
нынешнего блока; у владельца отеля собственного РТО не существует. Замена — одна строка в
`hotel_owner_details_lines`.

Заодно выяснилось, что `HOTEL_OWNER_DATA_FROM = '2026-09-14'` из комментария ревью был не
обходом незаполненных колонок, как я решил при разборе, а ровно этой бизнес-границей.
Константа вернулась, но проверяется по `order.created_at`, а не по `active_from` снапшота.

Тесты: 46 примеров, 0 падений, 1 предсуществующий pending. Добавлены контексты «when the
order was created before ГорТревел switched to hotel owner details» и «when the order
belongs to another operator organization»; успешный кейс дополнительно проверяет строку
`РТО 022770`. Решение про РТО отменено в следующем разделе — номер берётся из реестра ФСА.

### Правки после уточнений (22.09.2026)

Три изменения по решению постановщика и автора задачи, уже после ответов выше.

- **Реестровый номер берётся из реестра ФСА, а не у ГорТревела.** Строка печатает
  `package.hotel.register_record['id']` — номер самого объекта размещения (формат
  `С232025000761`), константа `HOTEL_OWNER_DETAILS_REGISTRY_NUMBER` удалена. Если номера у
  отеля нет, строка опускается, как строки адреса и ОГРН.
- **Дозаполнение убрано из синка.** `SyncWorker` вернулся к простому сравнению пяти полей:
  любое расхождение — новый снапшот. `fill_missing_attributes`,
  `optional_attribute_matches?` и константы `REQUIRED_ATTRIBUTES`/`OPTIONAL_ATTRIBUTES`
  удалены, спека воркера вернулась к прежним тестам.
- **Вместо этого разовая rake-таска** `one_time:backfill_hotel_organization_details`,
  прогоняется перед релизом и после него.

Отдельное решение по охвату таски: она заполняет **все** снапшоты отеля с совпадающим ИНН,
а не только последний. Обоснование — ОГРН принадлежит юрлицу, которое ИНН и определяет, и
для него неизменен, так что «датировать» его нечем; адрес мы в любом случае читаем текущий.
Снапшоты с другим ИНН оставляем пустыми: это предыдущие владельцы, и их ОГРН с адресом
взять неоткуда — заполнить их сегодняшними данными означало бы приписать одному юрлицу
реквизиты другого.

Тесты: 50 примеров, 0 падений, 1 предсуществующий pending. Новый файл
`spec/lib/tasks/backfill_hotel_organization_details_spec.rb` (5 примеров) по образцу
`backfill_payment_card_numbers_spec.rb`. В спеке декоратора появился контекст «when the
hotel has no number in the FSA registry», успешный кейс проверяет `С232025000761`.

Rubocop: у новых файлов остались ровно те же замечания, что у существующей пары
`backfill_payment_card_numbers` (`Metrics/BlockLength` у rake-таски,
`RSpec/DescribeClass`/`BeforeAfterAll`/`ExampleLength` у её спеки) — принятый в проекте
паттерн для `one_time`-тасок.

Покрытие `register_record['id']` проверено по Replica 22.09.2026: ключ заполнен у всех
17 716 отелей под синком, то есть строка «Реестровый номер» пропадать не будет.

**Сужение таски (22.09.2026).** После замера стало видно, что обход всего реестра — 17 222
запроса к ФСА в один поток, 3,5–5 часов на прогон, и это дважды вокруг релиза. Таска
переведена на отбор по заказам: берутся только отели отельных заказов ГорТревела по России,
созданных с 14.09.2026 (466 отелей, 444 со снапшотами, 512 записей — 5–10 минут). Остальным
отелям дозаполнение не нужно: ночной синк создаст им снапшот с заполненными полями, и заказы
с этого момента отрисуются сами. Цена решения — ~17 тысяч новых снапшотов от первого ночного
прогона, принято осознанно. **Отменено в следующем разделе:** переход на зеркало реестра
сделал полный обход дешёвым, отбор по заказам и эта цена больше не нужны.

### Переход на зеркало реестра (22.09.2026)

Таска переписана: вместо живых запросов к ФСА значения берутся из `fsa_registry_entries` —
локального зеркала того же реестра, которое `Fsa::RegistrySync` обновляет ежедневно. Отбор по
заказам ГорТревела больше не нужен: без HTTP прогон идёт секунды, поэтому заполняются все
снапшоты, а не 512 избранных.

Решение противоречит записи в «Out of scope», где зеркало было отклонено как источник, —
поэтому запись переписана, а не оставлена как есть. Отклонение остаётся в силе для самого
синка: телефона в зеркале нет, снапшот из него не собрать. Для разового дозаполнения двух
полей возражение «рассинхрон» снято замером: `Fsa::RegistrySync` кладёт `entry['addressList']`
и `entry['ownerOgrn']` ровно так же, как их отдаёт живой эндпоинт, и на отеле из VCR-кассеты
адрес совпал точным сравнением строк, а ОГРН пуст в обоих источниках.

Что это даёт против предыдущей версии: дозаполняются все 19 413 подходящих снапшотов вместо
512, ночной синк не создаёт ~17 тысяч новых строк, нагрузки на ФСА нет совсем, прогон
занимает секунды вместо 5–10 минут (и вместо 3,5–5 часов у версии с полным обходом API).

Остаточный риск — многоадресные записи: у 31% записей зеркала адресов несколько, фетчер и
таска берут первый, а совпадение порядка между showcase- и export-эндпоинтами проверить
нечем (ключа ФСА в dev-окружении нет). Среди отелей, которые реально рендерятся, таких 171
из 452. Последствие расхождения — новый снапшот от ночного синка (то есть поведение без
таски) и другой, но настоящий адрес того же объекта в договоре. Доля видна по числу новых
снапшотов после первого ночного прогона.

### Тип колонки и замеры расхождений (22.09.2026)

- `address` переведён в `text` правкой той же невыкаченной миграции: в зеркале уже есть адрес
  на 584 символа, а восемь — длиннее 400. Лимит 512 был наш, не реестра. Заодно ушли
  `length`-валидация на `address`, проверка длины в таске и отдельный счётчик пропущенных
  значений. `name` не трогали: его пишет синк из export-эндпоинта, максимум там 269.
- Миграция откачена и переприменена в dev и test, `db/schema.rb` снова сведён вручную к
  диффу в одну строку (дампер опять переписал файл целиком — известный дрейф версии).
- Уточнение к предыдущему разделу: заявление «значения совпадают побайтово» опиралось на один
  отель из VCR-кассеты. Прямая сверка на всём объёме даёт 96,5% (16 571 из 17 170 по `name`),
  так что несколько сотен снапшотов ночной синк всё же создаст. Это не отменяет перехода на
  зеркало — вместо ~17 тысяч лишних строк ожидаем сотни, — но ожидание «ноль дублей» неверно.
- Наблюдение за рамками задачи: у `HotelOrganizations::SyncWorker` нет защиты от значения,
  не влезающего в колонку. `create!` внутри `find_each` уронит весь батч из 1000 отелей.
  Сегодня не стреляет (максимумы из export-эндпоинта с запасом влезают), но чинить это —
  отдельная задача.

### Порядок выкатки

Порядок: выложить релиз (миграция и таска едут в нём же), прогнать
`rake one_time:backfill_hotel_organization_details`, включить флаг. Ждать ночного синка не
нужно — таска заполняет данные сама и не ходит в ФСА. Перед релизом её не прогнать: колонок
`ogrn`/`address` в проде ещё нет.

Второй прогон после ближайшей ночи имеет смысл только как страховка: он подберёт снапшоты,
созданные синком за эту ночь и почему-либо оставшиеся пустыми. Стоит секунды.

Пока таска не отработала, флаг включать бессмысленно, но и безопасно: снапшот без адреса и
ОГРН не проходит `hotel_owner_details_known?`, и договор печатает прежний текст
туроператора.

Замеры по Replica на 22.09.2026.

**Охват рендеринга.** Заказов ГорТревела с `search_type = 'hotel'`, созданных с 14.09.2026,
— 1346. Из них по отелям в России — 733, и у 711 из них есть снапшот владельца, то есть блок
получат ~97% подходящих заказов, остальные останутся на прежнем тексте (сценарий 3). Темп —
около 90 подходящих заказов в сутки. Реестровый номер есть у **всех** 17 716 отелей под
синком (`register_record->>'$.id'` заполнен в 100% случаев), так что строка не будет
пропадать.

**Объём таски.** Колонок `ogrn`/`address` на проде ещё нет — миграция не выкачена, поэтому
на момент прогона пустыми будут все 19 575 снапшотов. Из них запись в зеркале есть у 19 528,
и у 19 413 ИНН снапшота совпадает с `owner_inn` — это и есть объём заполнения. Адрес получат
все 19 413, ОГРН — 17 174 (88%; остальные ИП, у них ОГРН в реестре пуст). Оставшиеся ~160
строк — отели без записи в зеркале и снапшоты предыдущих владельцев. Запросов к ФСА нет
вовсе, прогон — секунды.

**Дублей быть не должно.** Ночной синк сравнивает живой ответ с последним снапшотом; после
таски значения там уже стоят и нормализованы так же, поэтому расхождения он не увидит. Это и
есть ответ на замечание ревью про 17 тысяч лишних строк — в отличие от версии с обходом
живого API, здесь дозаполнение получают все отели, а не только те, по которым уже есть
заказы. Остаточный риск (см. «Implementation decisions»): два эндпоинта ФСА согласны на
96,5% по `name`, плюс многоадресные записи, — поэтому несколько сотен снапшотов синк всё-таки
создаст.

Проверка заполнения — по последнему снапшоту каждого отеля, именно его берёт декоратор для
свежих заказов (таска заполняет и более ранние снапшоты того же владельца):
