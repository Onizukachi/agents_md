...
отеля есть засинканный из ФСА снапшот `HotelOrganization`, договор вместо блока
туроператора печатает короткий блок реквизитов владельца отеля: наименование (в обе
строки — «полное» и «сокращённое», так как ФСА не разделяет эти формы), адрес, ИНН, ОГРН
...
если заказ подходит (`hotel?`, `country&.id == Place::RUSSIA_ID`,
...
3. **Рендер в договоре.** `spec/decorators/order_decorator_spec.rb` — новый
   ...
   `receive_messages` на `hotel?`, `package`, `country`, флаг), покрывает сценарии 1-6:
...
Флаг включать **только после первого ночного прогона** `HotelOrganizations::RunnerWorker`
(1:00). У 17 222 отелей снапшот уже есть, и сразу после миграции `ogrn`/`address` у него
пустые, — при включённом флаге договор напечатает блок владельца без адреса и ОГРН, то
есть хуже прежнего текста. Точечно ускорить можно `RunnerWorker.perform_async` сразу
после релиза.

Объём (Replica, 21.09.2026): отелей под синк — 17 594, снапшотов — 19 574, отелей со
снапшотом — 17 222. Первый прогон создаст порядка 17 тысяч новых строк; дополнительной
нагрузки на ФСА не появляется, обход тот же. Охват ограничен реестром: `their_id` есть у
17 594 российских отелей из 74 767, по остальным договор сохраняет прежний текст
туроператора (сценарий 3).
